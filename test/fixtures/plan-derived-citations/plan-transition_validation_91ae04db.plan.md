...
|------------------------|-------------------------|
| User supplies **Before**, **After**, **`Plan.md`** | CLI `agentskeptic plan-transition --repo <path> --before <ref> --after <ref> --plan <path>`; optional `--workflow-id` defaulting to `wf_plan_transition`. |
...
| **CLI integration** | Spawn `agentskeptic plan-transition` on temp repo | stdout `WorkflowResult`, stderr rules. |
