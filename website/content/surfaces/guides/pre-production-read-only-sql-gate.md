...
Operational discipline is to run read-only checks against the same schema version you ship, with credentials scoped to SELECT, and to archive contract stdout (**Outcome Certificate v3**, **`schemaVersion: 3`**, **`failureSpine`**, **`evidenceCompleteness`**) plus the human report for later diffing.
