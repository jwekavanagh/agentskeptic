...
When your LangGraph trace lists every step as successful yet the Postgres row your workflow implied is still missing at verification time, treat the trace as narrative—not ledger proof. Export structured tool parameters, run read-only `SELECT`s against the same database, and expect ROW_ABSENT when the declared identity is absent.
