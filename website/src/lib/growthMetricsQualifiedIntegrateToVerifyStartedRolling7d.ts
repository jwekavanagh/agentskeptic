...
/**
 * Executable mirror — must match docs/growth-metrics-ssot.md §CrossSurface_ConversionRate_QualifiedIntegrateToVerifyStarted_Rolling7dUtc
 * (enforced by growthMetricsSqlParity.test.ts).
 */
export const CrossSurface_ConversionRate_QualifiedIntegrateToVerifyStarted_Rolling7dUtc_SQL = `WITH w AS (
...
export type CrossSurfaceQualifiedIntegrateToVerifyStartedRow = {
  d: number;
  n: number;
  rate: number | null;
};

export async function getCrossSurfaceQualifiedIntegrateToVerifyStartedRolling7d(): Promise<CrossSurfaceQualifiedIntegrateToVerifyStartedRow> {
  const rows = await dbTelemetry.execute(
    sql.raw(CrossSurface_ConversionRate_QualifiedIntegrateToVerifyStarted_Rolling7dUtc_SQL),
  );
  const row = rows[0] as CrossSurfaceQualifiedIntegrateToVerifyStartedRow | undefined;
  return {
    d: row?.d ?? 0,
    n: row?.n ?? 0,
    rate: row?.rate ?? null,
