# After copying from https://agentskeptic.com/integrate: for headless/CI machines run `agentskeptic funnel-anon pull` once (server-minted id; requires AGENTSKEPTIC_FUNNEL_ANON_ID unset). With a browser-issued uuid from /integrate or surface-impression, use `agentskeptic funnel-anon set <uuid>` instead (see docs/funnel-observability.md). Optional: export AGENTSKEPTIC_VERIFICATION_HYPOTHESIS for hypothesis tagging.
# PatternComplete: final verify uses temp DB path ($ADOPT_DB); see docs/first-run-integration.md §AdoptionComplete_PatternComplete.
# IntegrateSpineComplete: final activate+crossing (pack-led) on integrator SQLite after the guard; see docs/first-run-integration.md (Integrate spine normative).
set -euo pipefail
git -c core.autocrlf=false clone --depth 1 "${INTEGRATE_SPINE_GIT_URL:-https://github.com/jwekavanagh/agentskeptic.git}"
cd agentskeptic
# Pin Node dirname on PATH (validator sets INTEGRATE_SPINE_NODE). Use `node dist/cli.js` so argv matches partner scripts; Git Bash + direct `node.exe` + two absolute paths can mangle argv on some hosts.
INTEGRATE_SPINE_NODE="${INTEGRATE_SPINE_NODE:-$(command -v node)}"
export PATH="$(dirname "$INTEGRATE_SPINE_NODE"):$PATH"
npm install
npm run build
npm start
npm run first-run-verify
OUT="$(mktemp -u "${TMPDIR:-/tmp}/agentskeptic-integrate-mid-XXXXXXXX")"
ADOPT_DB="$(mktemp)"
trap 'rm -rf "$OUT" "$ADOPT_DB"' EXIT
node dist/cli.js activate --input test/fixtures/bootstrap-pack/input.json --db examples/demo.db --out "$OUT"
cp examples/demo.db "$ADOPT_DB"
node dist/cli.js --workflow-id wf_bootstrap_fixture --events "$OUT/events.ndjson" --registry "$OUT/tools.json" --db "$ADOPT_DB"
if [ -z "${AGENTSKEPTIC_VERIFY_DB:-}" ] || [ ! -f "${AGENTSKEPTIC_VERIFY_DB}" ] || [ ! -r "${AGENTSKEPTIC_VERIFY_DB}" ]; then
  echo "AgentSkeptic integrate spine: set AGENTSKEPTIC_VERIFY_DB to a readable SQLite file path (apply examples/integrate-your-db/required-sqlite-state.sql first). See docs/first-run-integration.md." >&2
  exit 1
fi
OUT2="$(mktemp -u "${TMPDIR:-/tmp}/agentskeptic-integrate-final-XXXXXXXX")"
trap 'rm -rf "$OUT" "$ADOPT_DB" "$OUT2"' EXIT
node dist/cli.js activate --input examples/integrate-your-db/bootstrap-input.json --db "$AGENTSKEPTIC_VERIFY_DB" --out "$OUT2"
node dist/cli.js crossing --workflow-id wf_integrate_spine --events "$OUT2/events.ndjson" --registry "$OUT2/tools.json" --db "$AGENTSKEPTIC_VERIFY_DB"
