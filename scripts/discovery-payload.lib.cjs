...
function renderLlmsTextFromPayload(payload) {
  ...
  const lines = [
    ...
    "- Decision evidence on disk: add `--proof <dir>` or `--write-decision-bundle <dir>`. The on-disk bundle is self-verifying — recipients run `agentskeptic decision-bundle validate <dir> [--public-key <path>]` and check that `integrity.selfVerifying === true`.",
    "- Full local proof (technical + decision bundles): add `--write-run-bundle <dir>` alongside `--write-decision-bundle` (two output directories). Pair `--sign-ed25519-private-key <path>` with either flag to write `manifest.sig.json`.",
