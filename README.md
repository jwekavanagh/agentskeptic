...
Adoption pointers: **[`docs/golden-path.md`](docs/golden-path.md)**. Authoritative CLI and behavior: **[`docs/workflow-verifier.md`](docs/workflow-verifier.md)**.
...
Postgres: use **`--postgres-url "postgresql://…"`** instead of **`--db <sqlitePath>`** (exactly one of the two). Copy-paste batch flags for SQLite live under **[Adoption integrator command](docs/workflow-verifier.md#adoption-integrator-command)**.
...
**When the row matches:** workflow result (excerpt; CLI prints full JSON to stdout):
...
Emit **one NDJSON line per tool observation** (see [Event line schema](docs/workflow-verifier.md#event-line-schema)), maintain **`tools.json`**, then run batch verification as documented in **[`docs/workflow-verifier.md`](docs/workflow-verifier.md)** (build step, flags, and streams). **[`docs/golden-path.md`](docs/golden-path.md)** lists role-based entry links.
...
The **`npm start`** driver runs the real CLI with inherited stdio (human report on **stderr**, **`WorkflowResult`** JSON on **stdout**). See [Human truth report](docs/workflow-verifier.md#human-truth-report).
...
### Success (`wf_complete`)
    ...
    { "schemaVersion": 15, "workflowId": "wf_complete", "status": "complete", "steps": [{ "seq": 0, "toolId": "crm.upsert_contact", "status": "verified" }] }
...
### Failure (`wf_missing`)
    ...
    { "schemaVersion": 15, "workflowId": "wf_missing", "status": "inconsistent", "steps": [{ "seq": 0, "toolId": "crm.upsert_contact", "status": "missing", "reasons": [{ "code": "ROW_ABSENT" }] }] }
...
| Doc | Purpose |
|-----|---------|
| [`docs/golden-path.md`](docs/golden-path.md) | Adoption pointers by role |
| [`docs/workflow-verifier.md`](docs/workflow-verifier.md) | Authoritative CLI and behavior reference (SSOT) |
| [`docs/adoption-validation-spec.md`](docs/adoption-validation-spec.md) | Adoption proof registry and validation IDs |
...
Run **`npm test`** from the repository root. It executes **`npm run build`**, Vitest, the pinned SQLite **`node:test`** list, **`node scripts/demo.mjs`**, then records and verifies **`artifacts/adoption-validation-verdict.json`** (see [`docs/adoption-validation-spec.md`](docs/adoption-validation-spec.md)). No Postgres required.
