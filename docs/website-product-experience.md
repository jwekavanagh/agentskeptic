...
- **Bundled verifier SSOT:** [`website/src/lib/bundledContractVerify.ts`](../website/src/lib/bundledContractVerify.ts) — `runBundledContractVerify` is the single path for bundled verification (`paste` and `scenarioFile`), calling `verifyWorkflow` from `agentskeptic`, then building + validating Outcome Certificate v3.
