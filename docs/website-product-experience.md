...
1. **`/`** — Homepage `<main>` text order is: `heroTitle`, then `homepageDecisionFraming`, then `heroSubtitle`, then hero CTAs (primary anchor to **`#try-it`**, secondary **`Link`** to **`discoveryAcquisition.slug`**), bundled **failure** terminal excerpt from `shareableTerminalFailureExcerpt(discoveryAcquisition.shareableTerminalDemo.transcript)`, then the trust strip from `buildHomeTrustStripLinks`, then **Try it**, **stakes** (`homeStakes`), **How it works**, **Fit and limits**, **Commercial** (`page.sections.ts` order). `homepageHero` why/what/when is not part of that `<main>` text stream. Causality and mechanism limits are covered by **`homepage-causality-invariant`**. The hero `data-testid="home-hero-cta-row"` has **exactly two** controls: primary **`#try-it`** with **`btn`**, secondary acquisition with **`btn secondary`**; **`/pricing` is not in the hero row** (pricing remains in the site header).
