...
The OSS composite [`.github/actions/agentskeptic-check`](../.github/actions/agentskeptic-check) accepts **`enforce-coverage-budget`**: when the input is exactly the string **`true`**, it appends **`--enforce-coverage-budget`** to **`check`** (ignored for **`enforce`** mode in v1). **`agentskeptic enforce`** does not implement coverage budgets in v1.
