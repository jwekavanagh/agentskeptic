...
- **Verification** — see **[`docs/testing.md`](testing.md)**. **`npm test`** is `node scripts/verify.mjs --profile=default` (no Postgres). **`npm run test:ci`** is `--profile=ci` (Postgres, Playwright, `commercial-enforce` with `--require-postgres`); requires Postgres 16+ and **`POSTGRES_ADMIN_URL`**, **`POSTGRES_VERIFICATION_URL`**. CI sets both; locally use the README Docker one-liner. **`npm install` does not run `tsc`** (no **`prepare`**).
