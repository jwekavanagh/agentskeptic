...
**CLI anonymous funnel telemetry:** On **`verify` / `quick`**, the CLI may best-effort POST **`verify_started`** and **`verify_outcome`** (same contracts as the operator SSOT). This is **not** part of verification correctness or OpenAPI integrator guarantees. **`AGENTSKEPTIC_TELEMETRY=0`** disables only those activation posts. Default OSS destination is the **canonical site origin** from anchor sync unless **`AGENTSKEPTIC_TELEMETRY_ORIGIN`** is set (required for some split deployments — see SSOT). Optional **`AGENTSKEPTIC_FUNNEL_ANON_ID`** (UUIDv4) forwards the anonymous id from the site beacon so those rows can join surface impressions in operator metrics (see [`funnel-observability-ssot.md`](funnel-observability-ssot.md); growth SQL in [`growth-metrics-ssot.md`](growth-metrics-ssot.md)).
