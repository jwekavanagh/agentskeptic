...
- **Vercel production:** Set `VERCEL_ENV=production` (Vercel default on Production). Set `NEXT_PUBLIC_APP_URL` to the canonical public origin (must match [`config/marketing.json`](../config/marketing.json) `productionCanonicalOrigin` per existing build parity checks).
